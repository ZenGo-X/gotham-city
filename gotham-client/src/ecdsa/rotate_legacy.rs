...
pub fn rotate_master_key(wallet: wallet::Wallet, client_shim: &api::ClientShim) -> wallet::Wallet {
    ...
    let res_body = requests::post(
        client_shim,
        &format!("{}/{}/first_legacy", ROT_PATH_PRE, id),
    )
    .unwrap();
    ...
    let res_body = requests::postb(
        client_shim,
        &format!("{}/{}/second_legacy", ROT_PATH_PRE, id.clone()),
        body,
    )
    .unwrap();
    ...
    let res_body = requests::postb(
        client_shim,
        &format!("{}/{}/third_legacy", ROT_PATH_PRE, id.clone()),
        body,
    )
    .unwrap();
    ...
    let res_body = requests::postb(
        client_shim,
        &format!("{}/{}/fourth_legacy", ROT_PATH_PRE, id.clone()),
        body,
    )
    .unwrap();
